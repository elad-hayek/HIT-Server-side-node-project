...
describe("Users Controller", () => {
  ...
  describe("POST /api/add", () => {
    it("should create a new user successfully", async () => {
      ...
      expect(usersService.addUser).toHaveBeenCalledWith(userData, "test-request-id");
    ...
    it("should return 400 when required fields are missing", async () => {
      ...
      const validationError = new ValidationError("Field 'last_name' is required and must be a string");
    ...
    it("should return 409 when user id already exists", async () => {
      ...
      const duplicateError = new DuplicateError("User with id 1 already exists");
  ...
  describe("GET /api/users", () => {
    it("should return array of users", async () => {
      ...
      expect(usersService.getAllUsers).toHaveBeenCalledWith("test-request-id");
  ...
  describe("GET /api/users/:id", () => {
    it("should return user with total_costs when costs-service responds", async () => {
      ...
      expect(usersService.getUserById).toHaveBeenCalledWith("1", "test-request-id");
      expect(res.status).toHaveBeenCalledWith(200);
